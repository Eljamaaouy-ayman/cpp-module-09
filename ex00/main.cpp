...
int main(int ac, char **av){
    ...
    int pos;
    if (ac != 2)
        return (std::cerr << "Error: could not open the file" << std::endl, 0);
    std::ifstream inFile("data.csv");
    if (!inFile.is_open())
        return (std::cerr << "Error can not open thee file data.csv" << std::endl, 0);
    ...
    if (line1.compare("date | value"))
        return (std::cerr << "Error syntax of the file" << av[1] << std::endl, 0);
    double value;
    while(std::getline(inFile1, line1))
    {
        if (parse_line(line1, value))
